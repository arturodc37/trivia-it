...
export default function HiddenAdminPage() {
  ...
  const fetchState = useCallback(async () => {
    ...
    const { data: gData } = await supabase.from("game").select("*").eq("id", 1).maybeSingle();
    if (gData) {
  ...
  const handleResetGame = async () => {
    if (confirm("¿Seguro que deseas reiniciar el juego a la sala de espera? (Los puntajes se reinician a 0)")) {
  ...
  const handleClearPlayers = async () => {
    if (confirm("¿Seguro que deseas eliminar todos los jugadores registrados para una nueva sesión limpia?")) {
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-slate-800">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/30 text-indigo-300 text-xs font-semibold mb-2">
            ...
            Panel Secreto del Organizador
          </div>
          <h1 className="text-3xl font-extrabold tracking-tight text-white flex items-center gap-3">
            Rapidagil IT <span className="text-xs px-2.5 py-1 rounded-md bg-slate-800 text-slate-400 font-mono font-normal">ADMIN #997784</span>
          </h1>
        </div>
